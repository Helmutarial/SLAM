...
def parseArgs():
    ...
    p.add_argument("--outputFolderNoWorld", help="Folder where to save the captured point clouds")
    ...
    args = p.parse_args()
    # Si no se pasa dataFolder, usar función externa para seleccionar carpeta
    if not args.dataFolder:
        import sys
        import os
        sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '../..')))
        from utils.file_utils import select_data_folder
        args.dataFolder = select_data_folder()
    return args

if __name__ == '__main__':
    args = parseArgs()
    # Crear carpeta de resultados automáticamente
    input_folder_name = os.path.basename(os.path.normpath(args.dataFolder))
    ...
    if args.outputFolderNoWorld:
        print("Saving point clouds to {0}".format(args.outputFolderNoWorld))
        for id in visu3D.pointCloudsNoWolrld:
            pc = visu3D.pointCloudsNoWolrld[id]
            filename = "{0}/{1}.ply".format(args.outputFolderNoWorld, id)
            o3d.io.write_point_cloud(filename, pc.cloud)'''
    ...
    for id in visu3D.pointClouds:
        ...
        # Extraer pose de cámara estimada (SLAM)
    ...
    print(f"✅ Poses guardadas en: {poses_path}")
